    ...
    if args.data_test == 'video':
        ...
        if checkpoint.ok:
            ...
            while not t.terminate():
                ...
                print('epochs time: {:.2f} h {:.2f} min '.format(int(epoch_time / 3600), epoch_time / 60),
                      'expected remain time: {:.2f} h {:.2f} min'.format(int(epoch_time * (args.epochs-t.optimizer.get_last_epoch()) / 3600),
                                                        epoch_time * (args.epochs-t.optimizer.get_last_epoch()) / 60 % 60))
            checkpoint.done()
